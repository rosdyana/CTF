...
def challenge1(r, arch):
    ...
    if arch == 'mips':
        ...
        os.system(
            f'/root/wabt/bin/wat2wasm {filename}.wast -o {filename}.wasm')
...
def challenge2(r, arch):
    ...
    if arch == 'riscv64':
        ...
        assembly = '\n'.join(map(lambda x: x[0], re.findall(
            ' *[0-9a-f]+: *[0-9a-f]+ *([^#\n]*)(#.*)?', assembly)))
...
{}
        ...
        os.system(
            f'/root/wabt/bin/wat2wasm {filename}.wast -o {filename}.wasm')
        os.system(
            f'/root/wabt/bin/wasm-interp {filename}.wasm --run-all-exports > {filename}.out')
